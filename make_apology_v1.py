...
Output: output/apology_never_came.wav (16-bit stereo, 44100 Hz)
"""
...
def pluck_arpeggio(bars: int = 4) -> list[float]:
    ...
    for bar in range(bars):
        ...
        for q in range(4):
            ...
            # Pan arp notes L/R alternating
            pan = -0.35 if q % 2 == 0 else 0.35
            gain_l = 0.32 * (1.0 - pan * 0.5)
            gain_r = 0.32 * (1.0 + pan * 0.5)
            # Will use stereo in the arrangement section
...
def mix_stereo(mono: list[float], offset: int,
...
mix_stereo(intro_drone, intro_start, 0.4, 0.4)

# Space texture — wide stereo
space_seg = space_texture[:samples_for(INTRO_BARS * 4)]
space_seg = fade_in(space_seg, duration_s=BAR * 2)
mix_into(master_L, space_seg, intro_start, gain=0.2)
mix_into(master_R, space_seg, intro_start, gain=0.25)
...
mix_stereo(intro_pad, intro_start, 0.3, 0.3)

# Reverse hit at bar 7 — something is coming
rev_offset = intro_start + samples_for(6 * 4)
mix_stereo(reverse_hit, rev_offset, 0.4, 0.4)

# Minimal hats fading in
intro_drums = drum_pattern_intro(INTRO_BARS)
intro_drums = fade_in(intro_drums, duration_s=BAR * 4)
mix_stereo(intro_drums, intro_start, 0.25, 0.25)

# Vocal whisper "oo" at bar 4 and 8
mix_stereo(chop_oo, intro_start + samples_for(3 * 4), 0.15, 0.18)
mix_stereo(chop_oo, intro_start + samples_for(7 * 4), 0.18, 0.15)
...
for rep in range(VERSE_BARS // 4):
    ...
    # Pan arp wider than center
    mix_into(master_L, arp, offset, gain=0.28)
    mix_into(master_R, arp, offset, gain=0.35)

# Sub bass — heartbeat
for bar in range(VERSE_BARS):
    bo = verse_start + samples_for(bar * 4)
    mix_stereo(sub_bass, bo, 0.4, 0.4)

# Verse drums — intimate
verse_drums = drum_pattern_verse(VERSE_BARS)
mix_stereo(verse_drums, verse_start, 0.45, 0.45)
...
mix_stereo(verse_reese, verse_start, 0.25, 0.25)
...
mix_stereo(verse_pad, verse_start, 0.3, 0.3)

# Vocal "ah" cry — scattered, the raw heart
for bar in [0, 2, 4, 6]:
    mix_stereo(chop_ah, verse_start + samples_for(bar * 4 + 2),
               0.2, 0.22)
# "oh" on bar 4 — the hollow
mix_stereo(chop_oh, verse_start + samples_for(3 * 4 + 3), 0.18, 0.2)
...
# Pitch riser — tension
mix_stereo(pitch_riser, build1_start, 0.5, 0.5)

# Noise riser layered
noise_seg = noise_riser[:samples_for(BUILD1_BARS * 4)]
mix_stereo(noise_seg, build1_start, 0.3, 0.3)

# Snare roll
build1_drums = drum_pattern_build(BUILD1_BARS)
mix_stereo(build1_drums, build1_start, 0.6, 0.6)
...
for bar in range(BUILD1_BARS):
    ...
    mix_stereo(chop_eh_stutter, bo, gain, gain)
...
if ee_offset > build1_start:
    mix_stereo(chop_ee, ee_offset, 0.4, 0.4)
...
mix_stereo(build_pad, build1_start, 0.3, 0.3)
...
mix_stereo(sub_boom, drop1_start, 0.85, 0.85)
mix_stereo(cinema_hit, drop1_start, 0.55, 0.55)
mix_stereo(chop_ee, drop1_start, 0.3, 0.3)  # Scream at impact
...
for bar in range(BREAK_BARS):
    ...
    for q in range(4):
        ...
        pluck = pluck_notes[note]
        pan = -0.45 if q % 2 == 0 else 0.45
        gain_l = 0.3 * (1.0 - pan * 0.5)
        gain_r = 0.3 * (1.0 + pan * 0.5)
        mix_into(master_L, pluck, bo + samples_for(q), gain=gain_l)
        mix_into(master_R, pluck, bo + samples_for(q), gain=gain_r)

# Light hats
for bar in range(BREAK_BARS):
    bo = break_start + samples_for(bar * 4)
    for q in range(4):
        mix_stereo(hat_closed, bo + samples_for(q), 0.12, 0.12)
...
mix_stereo(sub_break_drone, break_start, 0.3, 0.3)

# "oo" whisper chops — "Phantoms of us whisper"
for bar in [0, 2, 4, 6]:
    mix_stereo(chop_oo, break_start + samples_for(bar * 4 + 1),
               0.2, 0.22)

# "oh" — hollow between the whispers
for bar in [1, 3, 5]:
    mix_stereo(chop_oh, break_start + samples_for(bar * 4 + 3),
               0.15, 0.17)

# Space texture in break
break_space = space_texture[:samples_for(BREAK_BARS * 4)]
mix_into(master_L, break_space, break_start, gain=0.12)
mix_into(master_R, break_space, break_start, gain=0.15)
...
mix_stereo(harmonic_riser, build2_start, 0.55, 0.55)

# Noise riser layer
noise_seg2 = noise_riser[:samples_for(BUILD2_BARS * 4)]
mix_stereo(noise_seg2, build2_start, 0.35, 0.35)
...
if ee2_offset > build2_start:
    mix_stereo(chop_ee, ee2_offset, 0.45, 0.45)
...
mix_stereo(cinema_hit, drop2_start, 0.65, 0.65)
mix_stereo(clap, drop2_start, 0.5, 0.5)
mix_stereo(chop_ee, drop2_start, 0.35, 0.35)
...
for rep in range(DROP2_BARS // 4):
    ...
    mix_stereo(d2_drums, offset, 0.75, 0.75)
    for bar in range(4):
...
# Lead — same motif but more presence and wider stereo
for rep in range(DROP2_BARS // 4):
    offset = drop2_start + samples_for(rep * 16)
...
for bar in range(DROP2_BARS):
    ...
    mix_stereo(saw_chord, stab_offset, 0.22, 0.22)

# FM stabs every 2 bars
for bar in range(0, DROP2_BARS, 2):
    stab_offset = drop2_start + samples_for(bar * 4)
    mix_stereo(fm_hit, stab_offset, 0.2, 0.2)

# Dark drone underneath drop 2
for rep in range(DROP2_BARS // 4):
    offset = drop2_start + samples_for(rep * 16)
    seg = dark_drone[:samples_for(4 * 4)]
    mix_stereo(seg, offset, 0.22, 0.22)

# "ah" vocal — defiance
for bar in range(0, DROP2_BARS, 2):
    mix_stereo(chop_ah, drop2_start + samples_for(bar * 4 + 1),
               0.2, 0.22)

# Stutter in last 2 bars — time fracturing
stutter_off = drop2_start + samples_for((DROP2_BARS - 2) * 4)
mix_stereo(stutter, stutter_off, 0.35, 0.35)
mix_stereo(stutter, stutter_off + samples_for(4), 0.4, 0.4)
...
mix_into(master_L, outro_space, outro_start, gain=0.15)
mix_into(master_R, outro_space, outro_start, gain=0.18)

# Sparse hats fading
outro_drums = drum_pattern_intro(OUTRO_BARS)
outro_drums = fade_out(outro_drums, duration_s=BAR * 4)
mix_stereo(outro_drums, outro_start, 0.2, 0.2)
...
mix_stereo(outro_sub, outro_start, 0.25, 0.25)

# Final vocal — "ah" long — the apology that never came
# At bar 2
mix_stereo(chop_ah_long, outro_start + samples_for(1 * 4), 0.25, 0.28)
# At bar 4
mix_stereo(chop_ah_long, outro_start + samples_for(3 * 4), 0.2, 0.22)
# At bar 6 — last whisper
final_chop = chop_ah_long[:len(chop_ah_long)]
final_chop = fade_out(final_chop, duration_s=BEAT * 3)
mix_stereo(final_chop, outro_start + samples_for(5 * 4), 0.15, 0.17)
...
print("  Modules:  11 engine modules")
print("=" * 60)
