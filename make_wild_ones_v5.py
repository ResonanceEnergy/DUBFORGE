#!/usr/bin/env python3
"""DUBFORGE -- "Wild Ones" v5 (Electro House Remix — VIP Edition)

A DUBFORGE electro house remix built around the studio acapella of
"Wild Ones" by Flo Rida feat. Sia.  V5 extends V4 with:
...
Output:
    output/stems/wild_ones_v5_*.wav       — 12 individual stem WAVs
    output/wild_ones_v5.wav               — stereo mastered mixdown
    output/midi/wild_ones_v5.mid          — MIDI for all melodic parts
    output/serum2/wild_ones_v5_patches.json — Serum 2 patch reference
    output/ableton/Wild_Ones_V5.als       — ALS with automation + cue pts
"""
...
print("=" * 60)
print('  DUBFORGE -- "Wild Ones" V5 (Electro House Remix — VIP)')
print("=" * 60)
...
with open("output/serum2/wild_ones_v5_patches.json", "w") as f:
...
bank = FXPBank(name="WILD_ONES_V5_BANK", presets=list(fxp_presets.values()))
write_fxb(bank, "output/presets/WILD_ONES_V5_BANK.fxb")
...
# -- BASS --
print("  [3/14] Bass (sub + electro wobble)...")
...
print("  [4/14] Phi-harmonic sub reinforcement...")
...
print("  [5/14] Growl resampler bass (phi-modulated mid-bass)...")
...
# -- SUPERSAW CHORDS --
print("  [6/14] Supersaw chords (Ab - Cm - Fm - Db)...")
...
# -- HOOK LEAD MELODY --
print("  [7/14] Hook lead melody (pluck + FM)...")
...
print("  [8/14] Loading studio acapella...")
...
# -- RISERS + FX --
print("  [12/14] Risers, impacts & transitions...")
...
# v5: texture in intro
intro_tex = atmos_texture[:samples_for(INTRO_BARS * 4)]
...
# v5: texture in break
break_tex = atmos_texture[:samples_for(BREAK_BARS * 4)]
...
# v5: texture bed
bridge_tex = atmos_texture[:samples_for(BRIDGE_BARS * 4)]
...
# v5: texture dissolving
outro_tex = atmos_texture[:samples_for(OUTRO_BARS * 4)]
...
# ── Insight 38, 84: Parallel saturation on bass/growl ────────────
print("  Insight 38/84: Parallel saturation (Dojo 'Saturator Digital Clip')...")


def _parallel_saturate(signal: np.ndarray, drive: float = 3.0,
                       mix: float = 0.25) -> np.ndarray:
    """Digital hard-clip saturation blended with dry signal."""
    wet = np.tanh(signal * drive) / max(np.tanh(drive), 1e-9)
    return signal * (1.0 - mix) + wet * mix


stems["BASS"] = _parallel_saturate(stems["BASS"], drive=2.5, mix=0.20)
stems["GROWL"] = _parallel_saturate(stems["GROWL"], drive=4.0, mix=0.30)
stems["HOOK"] = _parallel_saturate(stems["HOOK"], drive=1.8, mix=0.12)
print("    BASS   -> parallel saturate (drive=2.5, mix=20%)")
print("    GROWL  -> parallel saturate (drive=4.0, mix=30%)")
print("    HOOK   -> parallel saturate (drive=1.8, mix=12%)")
...
for name in STEM_NAMES:
    buf = stems[name]
    path = f"output/stems/wild_ones_v5_{name}.wav"

    peak = float(np.max(np.abs(buf))) if len(buf) > 0 else 1.0
    gain = min(1.0, 0.95 / peak) if peak > 0 else 1.0

    # v5: Stereo output for stereo-processed stems
    if buf.ndim == 2 and buf.shape[1] == 2:
        int_L = np.clip(buf[:, 0] * gain * 32767.0, -32768, 32767).astype(np.int16)
        int_R = np.clip(buf[:, 1] * gain * 32767.0, -32768, 32767).astype(np.int16)
        stereo_int = np.empty(len(int_L) * 2, dtype=np.int16)
        stereo_int[0::2] = int_L
        stereo_int[1::2] = int_R
...
print("  Mastering (electro house preset — v5)...")
...
output_path = "output/wild_ones_v5.wav"
...
midi_path = "output/midi/wild_ones_v5.mid"
...
# ====================================================================
print("  Generating Enhanced Ableton Live project (v5)...")
...
for idx, name in enumerate(STEM_NAMES):
    stem_abs = os.path.abspath(f"output/stems/wild_ones_v5_{name}.wav")
...
project = ALSProject(
    ...
    tracks=als_tracks,
    scenes=als_scenes,
    notes="Wild Ones V5 (Electro House Remix — VIP Edition) | "
...
als_path = "output/ableton/Wild_Ones_V5.als"
...
stem_dir_size = sum(
    ...
    if f.startswith("wild_ones_v5_") and f.endswith(".wav")
)
...
print(f"  Serum 2:  output/serum2/wild_ones_v5_patches.json")
...
if _user_lib:
    ...
    _stems_dst = _user_lib / "Samples" / "DUBFORGE" / "Wild_Ones_V5" / "Stems"
    _stems_dst.mkdir(parents=True, exist_ok=True)
    for _f in Path("output/stems").glob("wild_ones_v5_*.wav"):
        shutil.copy2(_f, _stems_dst / _f.name)
        _installed += 1
    print(f"    Stems:   {_installed} -> {_stems_dst}")

    _midi_dst = _user_lib / "Samples" / "DUBFORGE" / "Wild_Ones_V5" / "MIDI"
    _midi_dst.mkdir(parents=True, exist_ok=True)
    for _f in Path("output/midi").glob("wild_ones_v5*.mid"):
...
print("  Done. Wild Ones V5 Remix — VIP edition pipeline complete.")
print()
