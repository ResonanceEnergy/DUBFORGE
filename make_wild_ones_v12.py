#!/usr/bin/env python3
"""Wild Ones V12 -- Fractal Expansion.

Evolves V11's MIDI ALS architecture with:
  1. Polyrhythmic drum patterns (quintuplets in bridge/VIP)
  2. VOCAL_CHOP track -- rhythmic pitched vocal hits
  3. Developing counter-melody that evolves per section
  4. Extended harmonic palette (add9, sus4 voicings)
  5. Double-time kick in VIP2 section
  6. Fibonacci-spaced filter automation
  7. 15 MIDI tracks + 2 returns
  8. Golden section breakdown (bar 89) with textural reset

Key: Ab minor | BPM: 127 | Bars: 144 (Fibonacci)

Output:
  - Wild_Ones_V12.als -> MIDI tracks (load Serum 2 presets)
  - wild_ones_v12.mid -> Standard MIDI file
  - presets/*.fxp -> Individual Serum 2 presets
  - presets/Wild_Ones_V12_BANK.fxb -> Preset bank
"""
...
# Chord voicings (V12: expanded palette with add9 & sus4)
CHORD_Ab = [MIDI["Ab3"], MIDI["C4"], MIDI["Eb4"]]
CHORD_Cm = [MIDI["C3"], MIDI["Eb3"], MIDI["G3"]]
CHORD_Fm = [MIDI["F3"], MIDI["Ab3"], MIDI["C4"]]
CHORD_Db = [MIDI["Db3"], MIDI["F3"], MIDI["Ab3"]]
# V12 extended voicings
CHORD_Ab_add9 = [MIDI["Ab3"], MIDI["Bb3"], MIDI["C4"], MIDI["Eb4"]]
CHORD_Fm_sus4 = [MIDI["F3"], MIDI["Bb3"], MIDI["C4"]]
CHORD_Eb = [MIDI["Eb3"], MIDI["G3"], MIDI["Bb3"]]
CHORDS = [CHORD_Ab, CHORD_Cm, CHORD_Fm, CHORD_Db]
CHORDS_EXT = [CHORD_Ab_add9, CHORD_Cm, CHORD_Fm_sus4, CHORD_Eb]
BASS_ROOTS = [MIDI["Ab1"], MIDI["C2"], MIDI["F2"], MIDI["Db2"]]
SUB_ROOTS = [MIDI["Ab0"], MIDI["C2"] - 12, MIDI["F2"] - 12,
             MIDI["Db2"] - 12]
...
SEC_CHORUS1 = 24;     CHORUS1_BARS = 13      # Fibonacci 13
SEC_BREAK = 37;       BREAK_BARS = 3         # Fibonacci 3
SEC_VERSE2 = 40;      VERSE2_BARS = 8
SEC_PRECHORUS2 = 48;  PRECHORUS2_BARS = 8
SEC_CHORUS2 = 56;     CHORUS2_BARS = 13      # Fibonacci 13
SEC_BRIDGE = 69;      BRIDGE_BARS = 8
SEC_VIP = 77;         VIP_DROP_BARS = 13     # Fibonacci 13
SEC_VIP2 = 90;        VIP2_DROP_BARS = 8     # Double-time VIP
SEC_FINAL = 98;       FINAL_CHORUS_BARS = 21 # Fibonacci 21
SEC_OUTRO = 119;      OUTRO_BARS = 25

SECTIONS = [
    ("INTRO",         SEC_INTRO,      INTRO_BARS),
    ("VERSE1",        SEC_VERSE1,     VERSE1_BARS),
    ("PRE_CHORUS1",   SEC_PRECHORUS1, PRECHORUS1_BARS),
    ("DROP1",         SEC_CHORUS1,    CHORUS1_BARS),
    ("BREAK",         SEC_BREAK,      BREAK_BARS),
    ("VERSE2",        SEC_VERSE2,     VERSE2_BARS),
    ("PRE_CHORUS2",   SEC_PRECHORUS2, PRECHORUS2_BARS),
    ("DROP2",         SEC_CHORUS2,    CHORUS2_BARS),
    ("BRIDGE_GOLDEN", SEC_BRIDGE,     BRIDGE_BARS),
    ("VIP_DROP",      SEC_VIP,        VIP_DROP_BARS),
    ("VIP2_DOUBLE",   SEC_VIP2,       VIP2_DROP_BARS),
...
# -- Counter-Melody V12: evolves across sections ----------------------
COUNTER_BASE = [
...
# V12: VIP counter-melody shifts up a 4th
COUNTER_VIP = [
    (2.0, "Db4", 0.75), (3.5, "C4", 0.5), (5.0, "Bb3", 0.75),
    (6.5, "Ab3", 0.5), (7.0, "F3", 0.75),
    (10.5, "G3", 0.75), (11.5, "Ab3", 0.5), (13.0, "Bb3", 0.75),
    (14.5, "C4", 0.5), (15.5, "Db4", 0.5),
    (17.0, "C4", 0.75), (19.0, "Bb3", 0.75), (21.0, "Ab3", 0.75),
    (22.5, "F3", 0.5), (23.5, "G3", 0.5),
    (25.0, "Ab3", 0.75), (26.5, "Bb3", 0.5), (28.5, "C4", 0.75),
    (30.0, "Db4", 0.75), (31.5, "Eb4", 0.5),
]

# -- Vocal Chop Pitches (V12 NEW) -------------------------------------
VOCAL_CHOP_PATTERN = [
    (0.0, "Eb4", 0.25), (0.5, "Ab4", 0.25), (1.0, "Eb4", 0.5),
    (2.0, "C4", 0.25), (2.5, "Db4", 0.25), (3.0, "Eb4", 0.5),
    (4.0, "Ab4", 0.25), (4.5, "Bb4", 0.25), (5.0, "Ab4", 0.5),
    (6.0, "F4", 0.25), (6.5, "Eb4", 0.25), (7.0, "C4", 0.5),
]
...
# Serum 2 preset mapping (V12: adds VOCAL_CHOP)
SERUM2_PRESETS = {
    ...
    "VOCAL_CHOP": ("DUBFORGE_Vocal_Fracture",        "Pitched vocal chop"),
...
# ======================================================================
# GALATCIA INTEGRATION — V12 section mapping → engine functions
# ======================================================================

GALATCIA_ROOT = DEFAULT_GALATCIA_ROOT
_GALATCIA_SAMPLES = GALATCIA_ROOT / "Samples" / "Samples"

# V12 section → role mapping for engine.galatcia.SectionInfo
_SECTION_ROLE: dict[str, str] = {
    "INTRO": "intro", "VERSE1": "verse", "PRE_CHORUS1": "build",
    "DROP1": "drop", "BREAK": "break", "VERSE2": "verse",
    "PRE_CHORUS2": "build", "DROP2": "drop", "BRIDGE_GOLDEN": "bridge",
    "VIP_DROP": "vip", "VIP2_DOUBLE": "vip", "FINAL_CHORUS": "drop",
    "OUTRO": "outro",
}

V12_SECTIONS: list[SectionInfo] = [
    SectionInfo(name=s[0], start_bar=s[1], n_bars=s[2],
                role=_SECTION_ROLE.get(s[0], "verse"))
    for s in SECTIONS
]

# MIDI pitch → drum sample key (GM mapping from _gen_drums)
_PITCH_TO_DRUM = {
...
def _gen_drums() -> list[ALSMidiNote]:
    """V12 drums: polyrhythmic quintuplets, tom fills, double-time VIP2."""
    ...
    def _tom_fill(bar: int) -> None:
        """Descending tom fill at section transitions."""
    ...
    for sec_name, sec_start, sec_bars in SECTIONS:
        for bar_offset in range(sec_bars):
            ...
            is_drop = sec_name in ("DROP1", "DROP2", "FINAL_CHORUS")
            is_vip = sec_name == "VIP_DROP"
            is_vip2 = sec_name == "VIP2_DOUBLE"
            is_build = "PRE_CHORUS" in sec_name

            # -- VIP2: double-time kick, snare on 2 and 4 ----------------
            if is_vip2:
                for eighth in range(8):
                    t = eighth * 0.5
                    _add(KICK, bar, t, 0.25,
                         _humanize_vel(110, t))
                _add(SNARE, bar, 1.0, 0.5, 115)
                _add(SNARE, bar, 3.0, 0.5, 115)
                _add(CLAP, bar, 1.0, 0.25, 100)
                _add(CLAP, bar, 3.0, 0.25, 100)
                ...
                if is_last_bar:
                    _snare_roll(bar, 3.0, 4.0, 90, 127)
                continue

            # -- HALFTIME VIP: snare on 3, quintuplet hats ----------------
            if is_vip:
                ...
                # V12: quintuplet hi-hats (5 over 4 beats)
                for quint in range(10):
                    t = quint * (4.0 / 10.0)
                    is_accent = quint % 5 == 0
                    _add(CLOSED_HH, bar, t, 0.2,
                         _humanize_vel(90 if is_accent else 65, t))
                ...
                if is_last_bar:
                    _tom_fill(bar)
                    _snare_roll(bar, 3.5, 4.0, 80, 127)
                continue
            ...
            # -- BRIDGE: polyrhythmic feel (quintuplet kick) --------------
            if sec_name == "BRIDGE_GOLDEN":
                _add(KICK, bar, 0.0, 0.5, 95)
                # Quintuplet kick overlay
                for quint in range(5):
                    t = quint * (4.0 / 5.0)
                    if t > 0.1:
                        _add(KICK, bar, t, 0.25,
                             _humanize_vel(60, t))
                _add(SNARE, bar, 2.0, 0.5, 90)
                ...
                if is_last_bar:
                    _snare_roll(bar, 2.0, 4.0, 60, 120)
                continue
...
def _gen_bass() -> list[ALSMidiNote]:
    """Bass -- root notes following chord progression."""
    notes: list[ALSMidiNote] = []
...
def _gen_sub() -> list[ALSMidiNote]:
    ...
    notes: list[ALSMidiNote] = []
    drop_sections = [
        (SEC_CHORUS1, CHORUS1_BARS), (SEC_CHORUS2, CHORUS2_BARS),
        (SEC_VIP, VIP_DROP_BARS), (SEC_VIP2, VIP2_DROP_BARS),
...
def _gen_growl() -> list[ALSMidiNote]:
    """Growl bass -- VIP sections and final chorus."""
    notes: list[ALSMidiNote] = []
    growl_sections = [
        (SEC_VIP, VIP_DROP_BARS), (SEC_VIP2, VIP2_DROP_BARS),
        (SEC_FINAL, FINAL_CHORUS_BARS),
    ]
    for sec_start, sec_bars in growl_sections:
        for bar_off in range(sec_bars):
            bar = sec_start + bar_off
            chord_idx = (bar_off // 2) % 4
...
def _gen_wobble() -> list[ALSMidiNote]:
    ...
    for sec_start, sec_bars in wobble_sections:
        for bar_off in range(sec_bars):
            bar = sec_start + bar_off
            chord_idx = (bar_off // 2) % 4
            root = BASS_ROOTS[chord_idx]
...
def _gen_riddim() -> list[ALSMidiNote]:
    """Riddim -- short staccato hits in VIP/VIP2/final."""
    notes: list[ALSMidiNote] = []
    riddim_sections = [
        (SEC_VIP, VIP_DROP_BARS), (SEC_VIP2, VIP2_DROP_BARS),
        (SEC_FINAL, FINAL_CHORUS_BARS),
    ]
    for sec_start, sec_bars in riddim_sections:
        for bar_off in range(sec_bars):
            bar = sec_start + bar_off
            chord_idx = (bar_off // 2) % 4
            root = BASS_ROOTS[chord_idx]
...
def _gen_formant() -> list[ALSMidiNote]:
    """Formant bass -- vowel-shifting, VIP/VIP2/final."""
    notes: list[ALSMidiNote] = []
    formant_sections = [
        (SEC_VIP, VIP_DROP_BARS), (SEC_VIP2, VIP2_DROP_BARS),
        (SEC_FINAL, FINAL_CHORUS_BARS),
    ]
    for sec_start, sec_bars in formant_sections:
        for bar_off in range(sec_bars):
            bar = sec_start + bar_off
            chord_idx = (bar_off // 2) % 4
            root = [MIDI["Ab2"], MIDI["C3"], MIDI["F2"],
...
def _gen_lead() -> list[ALSMidiNote]:
    """Hook/lead -- 30-note loop in all drop sections."""
    notes: list[ALSMidiNote] = []
    lead_sections = [
        (SEC_CHORUS1, CHORUS1_BARS), (SEC_CHORUS2, CHORUS2_BARS),
        (SEC_VIP, VIP_DROP_BARS), (SEC_VIP2, VIP2_DROP_BARS),
    ...
    for sec_start, sec_bars in lead_sections:
        for rep in range(max(1, sec_bars // 8)):
            base_beat = (sec_start + rep * 8) * 4
            for beat_off, note_name, dur in HOOK_MELODY:
                if base_beat + beat_off >= (sec_start + sec_bars) * 4:
                    break
...
def _gen_counter() -> list[ALSMidiNote]:
    """Counter-melody -- V12: evolving phrases per section."""
    notes: list[ALSMidiNote] = []
    for sec_start, sec_bars in [(SEC_CHORUS1, CHORUS1_BARS),
                                 (SEC_CHORUS2, CHORUS2_BARS)]:
        for rep in range(max(1, sec_bars // 8)):
            base_beat = (sec_start + rep * 8) * 4
            for beat_off, note_name, dur in COUNTER_BASE:
                if base_beat + beat_off >= (sec_start + sec_bars) * 4:
                    break
    ...
    # VIP/VIP2/Final use transposed counter-melody
    for sec_start, sec_bars in [(SEC_VIP, VIP_DROP_BARS),
                                 (SEC_VIP2, VIP2_DROP_BARS),
                                 (SEC_FINAL, FINAL_CHORUS_BARS)]:
        for rep in range(max(1, sec_bars // 8)):
            base_beat = (sec_start + rep * 8) * 4
            for beat_off, note_name, dur in COUNTER_VIP:
                if base_beat + beat_off >= (sec_start + sec_bars) * 4:
                    break
                vel = _humanize_vel(80, beat_off)
                notes.append(ALSMidiNote(
                    pitch=MIDI[note_name],
                    time=base_beat + beat_off,
                    duration=dur, velocity=vel))
...
def _gen_vocal_chop() -> list[ALSMidiNote]:
    """V12 NEW: Vocal chop -- rhythmic pitched hits in drops."""
    notes: list[ALSMidiNote] = []
    chop_sections = [
        (SEC_CHORUS1, CHORUS1_BARS), (SEC_CHORUS2, CHORUS2_BARS),
        (SEC_VIP, VIP_DROP_BARS), (SEC_VIP2, VIP2_DROP_BARS),
    ...
    for sec_start, sec_bars in chop_sections:
...
def _gen_chords() -> list[ALSMidiNote]:
    """Chords -- V12: extended voicings in VIP sections."""
    notes: list[ALSMidiNote] = []
    # Standard voicings in drops
    for sec_start, sec_bars in [(SEC_CHORUS1, CHORUS1_BARS),
                                 (SEC_CHORUS2, CHORUS2_BARS)]:
        for bar_off in range(sec_bars):
            bar = sec_start + bar_off
    ...
    # Extended voicings in VIP/VIP2/final
    for sec_start, sec_bars in [(SEC_VIP, VIP_DROP_BARS),
                                 (SEC_VIP2, VIP2_DROP_BARS),
                                 (SEC_FINAL, FINAL_CHORUS_BARS)]:
        for bar_off in range(sec_bars):
            bar = sec_start + bar_off
            chord_idx = (bar_off // 2) % 4
            for pitch in CHORDS_EXT[chord_idx]:
...
def _gen_arp() -> list[ALSMidiNote]:
    """Arp -- 16th-note chord arpeggiation in drops/bridge/final."""
    notes: list[ALSMidiNote] = []
    for sec_start, sec_bars in [(SEC_CHORUS1, CHORUS1_BARS),
                                 (SEC_CHORUS2, CHORUS2_BARS),
                                 (SEC_BRIDGE, BRIDGE_BARS),
...
def _gen_fx() -> list[ALSMidiNote]:
    """FX hits -- impacts on drops, stutter in builds."""
    notes: list[ALSMidiNote] = []
    # Impact hits on all drop entries
    for sec_start in [SEC_CHORUS1, SEC_CHORUS2, SEC_VIP,
                      SEC_VIP2, SEC_FINAL]:
...
def _build_track_automations() -> dict[str, list[ALSAutomation]]:
    ...
    chop_send: list[tuple[int, int, float]] = [
        ...
        (SEC_VIP, VIP_DROP_BARS, 0.5),
        (SEC_VIP2, VIP2_DROP_BARS, 0.55),
        (SEC_FINAL, FINAL_CHORUS_BARS, 0.45),
    ]
    auto["VOCAL_CHOP"] = [
...
def _build_serum2_presets() -> dict[str, FXPPreset]:
    ...
    # V12 NEW: Vocal Chop preset
    presets["VOCAL_CHOP"] = _preset("DUBFORGE_Vocal_Fracture", [
        (0, "OscA_WtPos", 0.65), (1, "OscA_Level", 0.8),
        (2, "OscA_Unison", 0.1875), (3, "OscA_Detune", 0.05),
        (4, "OscA_Pan", 0.5), (5, "OscA_Semi", 0.5),
        (6, "OscB_Level", 0.3), (7, "SubOsc_Level", 0.0),
        (8, "SubOsc_Shape", 0.0), (9, "Filter_Cutoff", 0.7),
        (10, "Filter_Res", 0.35), (11, "Filter_Type", 0.0),
        (12, "Env1_Attack", 0.001), (13, "Env1_Decay", 0.1),
        (14, "Env1_Sustain", 0.2), (15, "Env1_Release", 0.08),
        (16, "Master_Volume", 0.7), (17, "Master_Tune", 0.5),
        (18, "Macro1", 0.55), (19, "Macro2", 0.45),
    ])
...
def _build_als_project(
    ...
    # -- GALATCIA audio tracks (drums, FX, loops) ---------------------
    galatcia = integrate_galatcia_als(
        drum_notes=midi_data.get("DRUMS"),
        sections=V12_SECTIONS,
        pitch_map=_PITCH_TO_DRUM,
    )
    galatcia_tracks = galatcia["audio_tracks"]
    galatcia_notes = galatcia["note_text"]

    # Insert GALATCIA audio tracks before return tracks
    returns = [t for t in als_tracks if t.track_type == "return"]
    midi_tracks = [t for t in als_tracks if t.track_type != "return"]
    als_tracks = midi_tracks + galatcia_tracks + returns
    ...
    return ALSProject(
        ...
        notes=(f"DUBFORGE {TRACK_NAME} -- Fractal Expansion\n"
               f"Key: {KEY_ROOT} minor | BPM: {BPM} | "
               f"Bars: {TOTAL_BARS}\n"
               f"V12 features: Polyrhythmic drums, vocal chop track,\n"
               f"evolving counter-melody, extended chord voicings,\n"
               f"double-time VIP2 section, Fibonacci section lengths.\n"
...
def main() -> None:
    print(f"\n{'=' * 60}")
    print(f"  DUBFORGE -- {TRACK_NAME} -- Fractal Expansion")
    ...
    print(f"    Presets:    {preset_count} copied to "
          f"{output_dir / 'presets' / 'galatcia'}")
    rack_count = copy_racks(output_dir=str(output_dir))
    print(f"    Racks:      {rack_count} copied to "
          f"{output_dir / 'racks'}")
    ...
    # -- Summary ------------------------------------------------------
    print(f"\n{'=' * 60}")
    print(f"  DONE -- {TRACK_NAME} (Fractal Expansion)")
    print(f"{'=' * 60}")
    print(f"\n  V12 Evolution (from V11):")
    print(f"    - Polyrhythmic quintuplet drums in VIP/bridge")
    print(f"    - VOCAL_CHOP track (pitched rhythmic hits)")
    print(f"    - Evolving counter-melody (base + VIP transposition)")
    print(f"    - Extended chord voicings (add9, sus4) in VIP sections")
    print(f"    - VIP2 double-time section (double-time kick)")
    print(f"    - Fibonacci section lengths (3, 8, 13, 21)")
    print(f"    - Tom fills at transitions")
    print(f"\n  Stats:")
